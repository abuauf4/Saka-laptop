...
export function HomePageClient({
  ...
  const brandTitle = homepage.brandTitle;
  const brandCopy = homepage.brandCopy;
  const brandPointsData = homepage.brandPoints;
  const workflowStagesData = homepage.workflowStages;
  const tokoPhotosData = homepage.tokoPhotos;
  const deviceCategoriesData = homepage.deviceCategories;
  const faqsData = homepage.faqs;
  const closingTitle = homepage.closingTitle;
  const closingSubtitle = homepage.closingSubtitle;

  // WhatsApp number from Lokasi props
  ...
  // Logo from server props — no client fetch
      ...
      <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
        <SheetContent className="w-[300px] bg-background border-border p-0">
          <div className="flex items-center justify-between border-b border-border px-4 h-16">
            <div className="flex items-center gap-2">
              <img src={logoSrc} alt={lokasi.namaToko || "Logo"} className="h-8 w-8 rounded-lg object-cover" />
